...
- **Climbr** — carnet de bloc en salle (Sports), app d'Athanor Studio **en ligne depuis le 10 septembre 2026** : https://apps.apple.com/fr/app/climbr-progression-escalade/id6803155208. Carte « Now on the App Store » sur l'index, cliquable vers `/climbr` ; pas encore de métriques (notes, avis) à afficher.
  ...
  - **Mail de l'app : `climbr@athanor-studio.io`** (CTA et Support). Le studio, lui, est joignable sur `contact@athanor-studio.io` (index). `privacy.html` porte `contact@athanor-studio.io` depuis le 11 septembre 2026 (elle avait porté `climbr@` du 10 au 11 : c'est la page de TOUTES les apps du studio, elle prend l'adresse du studio). ⚠️ Sa section Climbr ne nomme PAS le fournisseur du modèle de détection — « our backend » seulement — et déclare Mixpanel et Tenjin, ajoutés à l'app le 9 septembre. Depuis le 11 septembre 2026 elle décrit aussi le classement — pseudo, photo de profil et totaux mensuels publiés par défaut, compte anonyme, retrait depuis le Profil, signalement — en restant générale, à la demande ; les parties communes (Why, Legal basis, With whom, Retention) couvrent les « community features ».
